...
class UpdateMsItemVarianceRequest extends FormRequest
{
    /**
    ...
    {
        /** @var MsItemVariance $msItemVariance */
        $msItemVariance = $this->route('msItemVariance');
        ...
        return [
            ...
            'name' => [
                ...
                'string',
                'max:100',
                Rule::unique(MsItemVariance::class, 'name')
                    ->where(fn ($query) => $query
                        ->where('is_deleted', 0)
                        ->where('item_id', $this->integer('item_id')))
                    ->ignore($msItemVariance->row_id, 'row_id'),
            ],
