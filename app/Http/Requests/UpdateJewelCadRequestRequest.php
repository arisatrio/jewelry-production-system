...
class UpdateJewelCadRequestRequest extends FormRequest
{
    /**
    ...
    protected function prepareForValidation(): void
    {
        ...
        $this->merge([
            'doc_no' => $this->filled('doc_no')
                ? $this->string('doc_no')->trim()->toString()
                : null,
    ...
    {
        $requestModel = $this->route('jewelcad');
        ...
        return [
            'doc_no' => [
                ...
                'string',
                'max:50',
                Rule::unique(JewelCadRequest::class, 'doc_no')
                    ->ignore($requestModel?->getKey(), $requestModel?->getKeyName() ?? 'row_id')
            ...
            'details.*.spk_id' => [
                ...
                Rule::exists(Production::class, 'row_id')->where(
                    fn ($query) => app(JewelCadSpkEligibility::class)->applySelectableScope(
                        $query,
                        $requestModel instanceof JewelCadRequest ? (int) $requestModel->row_id : null,
                    ),
                ),
            ],
    ...
    {
        return [
            'doc_no.required' => 'Nomor request wajib diisi.',
            'doc_no.unique' => 'Nomor request sudah digunakan.',
