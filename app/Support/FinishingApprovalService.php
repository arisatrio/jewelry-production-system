...
class FinishingApprovalService
{
    ...
    public function isOpen(FinishingHandmade $document): bool
    {
    ...
    public function isPendingManager(FinishingHandmade $document): bool
    {
        $status = strtoupper(trim((string) $document->status));
    ...
    public function isPendingComplete(FinishingHandmade $document): bool
    {
        $status = strtoupper(trim((string) $document->status));
    ...
    public function isDone(FinishingHandmade $document): bool
    {
        $status = strtoupper(trim((string) $document->status));
     ...
     * }
     */
    public function abilitiesFor(FinishingHandmade $document, ?User $user): array
    {
    ...
    public function submit(FinishingHandmade $document, string $actor): FinishingHandmade
    {
        ...
        return DB::connection('third')->transaction(function () use ($document, $actor): FinishingHandmade {
    ...
    public function managerApprove(FinishingHandmade $document, string $actor, ?string $notes = null): FinishingHandmade
    {
        if (! $this->isPendingManager($document)) {
            throw new InvalidArgumentException('Dokumen harus berstatus Pengajuan sebelum di-approve.');
        }

        return DB::connection('third')->transaction(function () use ($document, $actor, $notes): FinishingHandmade {
            $this->writeApprovalLog(
                $document,
                self::STATUS_MANAGER,
                self::APPROVE_OK,
    ...
    public function complete(FinishingHandmade $document, string $actor, ?string $notes = null): FinishingHandmade
    {
     ...
     */
    public function footerColumns(FinishingHandmade $document, string $fallbackCreator = '-'): array
    {
     ...
     * }
     */
    public function map(FinishingHandmade $document): array
    {
    ...
    public function resolveKey(FinishingHandmade $document): string
    {
    ...
    public function statusLabelFor(FinishingHandmade $document): string
    {
        if ($this->isOpen($document)) {
            return 'Open';
        }
     ...
     */
    public function history(FinishingHandmade $document): array
    {
    ...
    public function statusLabels(): array
    {
        $defaults = [
            ...
            FinishingHandmade::STATUS_TO_PPIC => 'Serahkan ke PPIC',
    ...
    private function normalizedStatus(FinishingHandmade $document): string
    {
        if ($this->isOpen($document)) {
            return 'OPEN';
        }
    ...
    private function labelFor(string $key, FinishingHandmade $document): string
    {
        return match ($key) {
            self::KEY_DONE => 'Done',
