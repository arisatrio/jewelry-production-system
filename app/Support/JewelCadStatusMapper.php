...
class JewelCadStatusMapper
{
    ...
    public function __construct(
        private readonly JewelCadApprovalService $approvalService,
    ) {}
     ...
     * }
     */
    public function map(JewelCadRequest $request): array
    {
    ...
    public function resolveKey(JewelCadRequest $request): string
    {
        if ($this->approvalService->isDone($request)) {
            return self::KEY_DONE;
        }

        if ($this->approvalService->isPendingComplete($request)) {
            return self::KEY_MANAGER;
        }

        if ($this->approvalService->isPendingManager($request)) {
