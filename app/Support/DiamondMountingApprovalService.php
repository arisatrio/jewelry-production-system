...
class DiamondMountingApprovalService
{
    public const DOC_NAME = 'diamondmounting';

    public const STATUS_SUBMITTED = 'DMT010';

    public const STATUS_MANAGER = 'DMT030';
    ...
    public function isOpen(DiamondMounting $document): bool
    {
    ...
    public function isPendingManager(DiamondMounting $document): bool
    {
        $status = strtoupper(trim((string) $document->status));
    ...
    public function isPendingComplete(DiamondMounting $document): bool
    {
        $status = strtoupper(trim((string) $document->status));
    ...
    public function isDone(DiamondMounting $document): bool
    {
        $status = strtoupper(trim((string) $document->status));
     ...
     * }
     */
    public function abilitiesFor(DiamondMounting $document, ?User $user): array
    {
    ...
    public function submit(DiamondMounting $document, string $actor): DiamondMounting
    {
        ...
        return DB::connection('third')->transaction(function () use ($document, $actor): DiamondMounting {
    ...
    public function managerApprove(DiamondMounting $document, string $actor, ?string $notes = null): DiamondMounting
    {
        if (! $this->isPendingManager($document)) {
            throw new InvalidArgumentException('Dokumen harus berstatus Pengajuan sebelum di-approve.');
        }

        return DB::connection('third')->transaction(function () use ($document, $actor, $notes): DiamondMounting {
            $this->writeApprovalLog(
                $document,
                self::STATUS_MANAGER,
                self::APPROVE_OK,
    ...
    public function complete(DiamondMounting $document, string $actor, ?string $notes = null): DiamondMounting
    {
     ...
     */
    public function footerColumns(DiamondMounting $document, string $fallbackCreator = '-'): array
    {
     ...
     * }
     */
    public function map(DiamondMounting $document): array
    {
    ...
    public function resolveKey(DiamondMounting $document): string
    {
    ...
    public function statusLabelFor(DiamondMounting $document): string
    {
        if ($this->isOpen($document)) {
            return 'Open';
        }
     ...
     */
    public function history(DiamondMounting $document): array
    {
    ...
    public function statusLabels(): array
    {
        $defaults = [
            ...
            DiamondMounting::STATUS_TO_CRAFTSMAN => 'Pengajuan',
            DiamondMounting::STATUS_FROM_CRAFTSMAN => 'Pengajuan',
            DiamondMounting::STATUS_TO_PPIC => 'Serahkan ke PPIC',
            DiamondMounting::STATUS_DONE => 'Completed',
        ];
    ...
    private function normalizedStatus(DiamondMounting $document): string
    {
        if ($this->isOpen($document)) {
            return 'OPEN';
        }
    ...
    private function labelFor(string $key, DiamondMounting $document): string
    {
        return match ($key) {
            self::KEY_DONE => 'Done',
