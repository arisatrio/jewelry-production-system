...
import {
    PolesChromeSelectSpkDialog,
    type PolesChromeSelectedSpk,
} from '@/components/poles-chrome/poles-chrome-select-spk-dialog';
...
type PolesChromeFormValues = {
    ...
    spk: PolesChromeSpkForm | null;
};

type PolesChromeFormProps = {
    title: string;
...
function emptySpk(): PolesChromeSpkForm {
    return {
        spk_id: '',
        spk_no: '',
        spk_type: '',
...
function selectedSpkItemSummary(spk: PolesChromeSpkForm | null): string {
    if (spk === null) {
        return '—';
    }
...
export function PolesChromeForm({
    title,
    formDocumentNo,
    submitLabel,
    cancelHref,
    ...
    statusItemOptions,
    initialValues,
}: PolesChromeFormProps) {
    const { data, setData, post, put, processing, errors, transform } =
        useForm<PolesChromeFormValues>(initialValues);
    ...
    const handleSelectedSpk = (selected: PolesChromeSelectedSpk) => {
